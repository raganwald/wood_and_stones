class GameNotifier < ActionMailer::Base
  def game(game)
    ...
    unless (__126937012295297__ = game.last_black_notification and (__126937012295297__ >= game.last_action_updated_i)) then
      mail_to(game, game.black)
      game.update_attribute(:last_black_notification, game.last_action_updated_i)
    end
    unless (__126937012280403__ = game.last_white_notification and (__126937012280403__ >= game.last_action_updated_i)) then
  ...
  def mail_to(game, user)
    ...
    secret = Secret.first(:conditions => ({ :target_id => (game.id), :target_class => (game.class.name), :user_id => (user.id) }))
