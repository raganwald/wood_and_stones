class Action::Move < Action::Gameplay
  ...
  def location
    @location ||= (__126754971626295__ = self.position
    if __126754971626295__.kind_of?(String) then
      RewriteRails::ExtensionMethods::String.to_location(__126754971626295__)
    else
      __126754971626295__.to_location
    end)
  end
  def removed
  ...
  def place_stone
    (dead_stones = (__126754971564068__ = self.before.legal_moves_for(self.player).detect do |its|
      (its.location == self.location)
    end and __126754971564068__.dead_stones) and (self.removed = dead_stones
