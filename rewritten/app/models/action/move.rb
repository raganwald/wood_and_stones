class Action::Move < Action::Gameplay
  ...
  def location
    @location ||= (__126741192491356__ = self.position
    if __126741192491356__.kind_of?(String) then
      RewriteRails::ExtensionMethods::String.to_location(__126741192491356__)
    else
      __126741192491356__.to_location
    end)
  end
  def removed
  ...
  def place_stone
    (dead_stones = (__126741192383667__ = self.before.legal_moves_for(self.player).detect do |its|
      (its.location == self.location)
    end and __126741192383667__.dead_stones) and (self.removed = dead_stones
