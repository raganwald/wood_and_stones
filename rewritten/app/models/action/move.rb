class Action::Move < Action::Gameplay
  ...
  def location
    @location ||= (__126671987427591__ = self.position
    if __126671987427591__.kind_of?(String) then
      RewriteRails::ExtensionMethods::String.to_location(__126671987427591__)
    else
      __126671987427591__.to_location
    end)
  end
  def removed
  ...
  def place_stone
    (dead_stones = (__126671987422232__ = self.before.legal_moves_for(self.player).detect do |its|
      (its.location == self.location)
    end and __126671987422232__.dead_stones) and (self.removed = dead_stones
