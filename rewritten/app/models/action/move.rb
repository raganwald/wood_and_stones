class Action::Move < Action::Gameplay
  ...
  def location
    @location ||= (__126721699032184__ = self.position
    if __126721699032184__.kind_of?(String) then
      RewriteRails::ExtensionMethods::String.to_location(__126721699032184__)
    else
      __126721699032184__.to_location
    end)
  end
  def removed
  ...
  def place_stone
    (dead_stones = (__126721698970004__ = self.before.legal_moves_for(self.player).detect do |its|
      (its.location == self.location)
    end and __126721698970004__.dead_stones) and (self.removed = dead_stones
