class Action::Move < Action::Gameplay
  ...
  def location
    @location ||= (__126695721158734__ = self.position
    if __126695721158734__.kind_of?(String) then
      RewriteRails::ExtensionMethods::String.to_location(__126695721158734__)
    else
      __126695721158734__.to_location
    end)
  end
  def removed
  ...
  def place_stone
    (dead_stones = (__126695721126684__ = self.before.legal_moves_for(self.player).detect do |its|
      (its.location == self.location)
    end and __126695721126684__.dead_stones) and (self.removed = dead_stones
